...
# Fallback in case the variable is not set
...
if [ -n "$COMPOSE_SERVICES" ]; then
    COMPOSE_SERVICES_DOTS=$(echo "$COMPOSE_SERVICES" | sed "s/,/.${DOMAIN},/g" | sed "s/$/.${DOMAIN}/")
else
...
# 6. Combine everything, including the base custom domain,  into a raw string
RAW_NO_PROXY="${BASE_NO_PROXY},${COMPOSE_SERVICES},${COMPOSE_SERVICES_DOTS},${DOMAIN},.${DOMAIN},.dns.podman,${CIDR_BLOCKS}"
